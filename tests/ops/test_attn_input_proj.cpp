...
int run_nvfp4_target_case(DevicePackedWeight& parent, std::int32_t tokens,
    ...
    if (tokens == 1024 && policy == ops::LinearPolicy::AllowA4) {
        ...
        cuda_check(cudaGraphInstantiate(&exec, graph, 0), "instantiate NVFP4 attention projection graph");
        cuda_check(cudaGraphLaunch(exec, stream), "launch NVFP4 attention projection graph first replay");
        cuda_check(cudaGraphLaunch(exec, stream), "launch NVFP4 attention projection graph second replay");
        cuda_check(cudaStreamSynchronize(stream), "synchronize NVFP4 attention projection graph replay");
        cuda_check(cudaGraphExecDestroy(exec), "destroy NVFP4 attention projection graph executable");
