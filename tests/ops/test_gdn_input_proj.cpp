...
int run_nvfp4_case(DevicePackedWeight& parent, std::int32_t tokens, ops::LinearPolicy policy) {
    ...
    if (tokens == 1024 && policy == ops::LinearPolicy::AllowA4) {
        ...
        cuda_check(cudaStreamEndCapture(stream, &graph), "end NVFP4 GDN projection graph capture");
        cuda_check(cudaGraphInstantiate(&exec, graph, 0), "instantiate NVFP4 GDN projection graph");
        cuda_check(cudaGraphLaunch(exec, stream), "launch NVFP4 GDN projection graph first replay");
        cuda_check(cudaGraphLaunch(exec, stream), "launch NVFP4 GDN projection graph second replay");
        cuda_check(cudaStreamSynchronize(stream), "synchronize NVFP4 GDN projection graph replay");
        cuda_check(cudaGraphExecDestroy(exec), "destroy NVFP4 GDN projection graph executable");
