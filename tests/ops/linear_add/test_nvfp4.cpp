...
int run_shape(std::int32_t n, std::int32_t k, std::uint32_t seed) {
    ...
    for (const Invocation invocation : invocations) {
        ...
        if (invocation.tokens == 1024) {
            ...
            cuda_check(cudaStreamEndCapture(stream, &graph), "end NVFP4 linear_add graph capture");
            cuda_check(cudaGraphInstantiate(&exec, graph, 0), "instantiate NVFP4 linear_add graph");
            ...
            cuda_check(cudaGraphLaunch(exec, stream), "launch NVFP4 linear_add graph second replay");
